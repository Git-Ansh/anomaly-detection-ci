...
Softmax confidence thresholding achieves 90\% accuracy among retained predictions but retains only 12.8\% of bugs (requiring DeBERTa confidence $> 0.99$). It provides no formal coverage guarantee and discards the remaining 87\% entirely rather than providing shortlists. Ensemble voting improves overall accuracy to 67.1\% by selecting the higher-confidence model when predictions disagree, and the agree/disagree partition identifies reliable predictions (85.7\% vs.\ 50.7\%); however, it produces no set-valued output and no coverage guarantee. Single-model RAPS (DeBERTa or XGBoost) provides formal marginal coverage guarantees and useful prediction sets, but applies a uniform threshold that cannot distinguish easy from hard instances. MACCP combines reliability identification from ensemble agreement, formal coverage guarantees from conformal prediction, and per-group conditioning from Mondrian CP. The agree partition achieves the tightest sets (mean 2.30) with the highest singleton rate (61.2\%) of any configuration.

We initially explored multi-stage cascade architectures with confidence gating between stages (e.g., noise filtering followed by component assignment). Empirical analysis revealed that upstream stage predictions provide no additional signal to downstream stages when both share text features, a conditional independence result that motivated the shift to parallel cross-model agreement rather than sequential information passing.
...
MACCP partitions bugs into three tiers: AUTO-TRIAGE (29\%, agree singletons at 90\% accuracy), REVIEW SHORTLIST (10\%, agree non-singletons, developer picks from 2--3 options), and NEEDS CAREFUL REVIEW (56\%, disagree group, mean set size 8.52). The 10\% error rate among auto-triaged singletons is substantially lower than the 37--44\% human triager misassignment rate~\cite{jeong2009improving}. A rule-based explanation layer surfaces each model's top prediction and the historical accuracy of the partition.

\subsection{Cost-Benefit Analysis}

Maintaining two diverse classifiers has engineering overhead. Whether MACCP's 29\% automation rate justifies this cost depends on triage volume. With XGBoost RAPS alone, 20.1\% of predictions are singletons at 93.6\% accuracy. MACCP adds 9 percentage points of automation (29\% vs 20\%) at the cost of deploying and maintaining a transformer model. For high-volume projects processing hundreds of bugs daily, this additional automation represents significant time savings. For smaller projects, single-model conformal prediction may suffice. The primary computational cost is DeBERTa inference, approximately 50ms per bug on a single GPU. For organizations not already running a transformer model, this represents a non-trivial infrastructure addition.

We acknowledge that these results are specific to open-source Bugzilla-based projects with rich metadata. Whether the agreement signal provides similar value in commercial settings with different metadata structures (e.g., Jira, Azure DevOps) remains an open question.

\subsection{Why Metadata XGBoost Outperforms DeBERTa}

XGBoost achieves 66.0\% versus DeBERTa's 54.5\% under the 15-year temporal split because metadata features (product encoding, reporter history) are temporally invariant, while text vocabulary drifts substantially. This extends Grinsztajn et al.~\cite{grinsztajn2022tree} to a temporal shift setting: in long-lived software projects, metadata-based models may be more reliable than text-based models for triage.
...
In practice, our results suggest that architectural diversity in deployed classifiers provides a free, powerful conditioning signal for uncertainty quantification. When teams already deploy multiple models, as is common in production triage systems, cross-model agreement can be extracted at minimal additional cost and used to partition predictions into reliability tiers with formal per-group coverage guarantees.
