...
More broadly, when development teams already deploy multiple classifiers, as is increasingly common in production triage systems~\cite{chen2025triangle,xu2026whatscode}, cross-model agreement can be extracted at zero additional training cost and used as a conditioning signal for uncertainty quantification with formal guarantees.
...
Overall, the two models agree on 46.7\% of test bugs (11,913 / 25,499). When they agree, accuracy is 85.7\%; when they disagree, DeBERTa accuracy drops to 27.1\% while XGBoost retains 48.7\%. An oracle selecting the correct model when they disagree would achieve 80.4\%. Critically, on 57\% of the bugs where DeBERTa predicts the wrong component, XGBoost predicts the correct one, confirming that the two models exhibit complementary rather than correlated failure patterns. All within-bin gaps are statistically significant (chi-squared test, $p < 0.001$ for all bins on both datasets), confirming these differences are not due to sampling variation.
